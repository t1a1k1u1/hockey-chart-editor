...
func get_row_y(row: int) -> float:
	## Row 0-2: TOP, Row 3: NORMAL, Row 4-10: Vertical
	## Separators between groups: after row 2, after row 3
	## Actually separators: between TOP group rows, between groups
	var sep_count = 0
	...
	return row * TRACK_HEIGHT + sep_count * SEP_HEIGHT
