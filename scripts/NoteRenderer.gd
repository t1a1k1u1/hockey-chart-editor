...
func draw_note(canvas: CanvasItem, note: Dictionary, scroll_offset: float, pixels_per_second: float, is_selected: bool, grid_sec: float, col_width: float, content_offset_x: float, header_height: float) -> void:
	var t = note.get("type", "normal")
	var color = get_note_color(note)
	var col = _get_note_col(note)
	var cx = content_offset_x + col * col_width + col_width * 0.5
	var center_y = header_height + (note["time"] - scroll_offset) * pixels_per_second

	match t:
		"normal", "top", "vertical":
			_draw_normal_note(canvas, cx, center_y, color, is_selected, col_width, grid_sec, pixels_per_second)
		"long_normal", "long_top", "long_vertical":
			var y2 = header_height + (note.get("end_time", note["time"] + 0.5) - scroll_offset) * pixels_per_second
			_draw_long_note(canvas, cx, center_y, y2, color, is_selected, col_width)
		"chain":
			_draw_chain_note(canvas, note, scroll_offset, pixels_per_second, cx, color, is_selected, col_width, grid_sec, header_height)
...
func _draw_chain_note(canvas: CanvasItem, note: Dictionary, scroll_offset: float, pixels_per_second: float, cx: float, color: Color, is_selected: bool, col_width: float, grid_sec: float, header_height: float) -> void:
	...
	for i in range(count):
		...
		var yi = header_height + (t_i - scroll_offset) * pixels_per_second
		...
		if i == count - 1 and last_long and last_end_time > t_i:
			var y2 = header_height + (last_end_time - scroll_offset) * pixels_per_second
